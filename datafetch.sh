...
get_gpu() {
    ...
    local gpus best_line line clean cpu_model gpu_type
    ...
    # CPU model, useful for some AMD APU mappings
    if command -v lscpu >/dev/null 2>&1; then
        cpu_model=$(lscpu | awk -F: '/Model name:/ {gsub(/^[ \t]+/, "", $2); print $2; exit}')
        cpu_model=$(printf '%s\n' "$cpu_model" | sed 's/ with Radeon Graphics//')
    else
        cpu_model=$(grep -m1 'model name' /proc/cpuinfo 2>/dev/null | cut -d: -f2- | sed 's/^[[:space:]]*//' | sed 's/ with Radeon Graphics//')
    fi

    # 1. Prefer NVIDIA dedicated GPU
    best_line=$(printf '%s\n' "$gpus" | grep -i 'NVIDIA' | head -n1)

    # 2. Prefer obvious AMD dedicated GPU
    if [ -z "$best_line" ]; then
        best_line=$(printf '%s\n' "$gpus" \
            | grep -Ei 'Radeon|AMD|ATI' \
            | grep -Evi 'Renoir|Cezanne|Rembrandt|Phoenix|Barcelo|Picasso|Raven|Ryzen|Integrated|Vega [0-9]+ Graphics|Radeon Graphics' \
            | head -n1)
    fi

    # 3. Otherwise first AMD
    if [ -z "$best_line" ]; then
        best_line=$(printf '%s\n' "$gpus" | grep -Ei 'Radeon|AMD|ATI' | head -n1)
    fi

    # 4. Then Intel
    if [ -z "$best_line" ]; then
        best_line=$(printf '%s\n' "$gpus" | grep -i 'Intel' | head -n1)
    fi
    ...
    # Remove PCI address and revision
    line=$(printf '%s\n' "$best_line" \
    ...
    if printf '%s\n' "$line" | grep -qi 'NVIDIA'; then
        clean=$(printf '%s\n' "$line" \
            ...
            | sed -E 's/.*NVIDIA[[:space:]]*//' \
            | sed -E 's/[[:space:]]*\[.*//' \
            | sed -E 's/[[:space:]]+$//')

        gpu_type="[Dedicated]"
        [ -n "$clean" ] && echo "$clean $gpu_type" || echo "NVIDIA GPU $gpu_type"
        return
    fi

    # AMD cleanup
    if printf '%s\n' "$line" | grep -Eqi 'AMD|ATI|Radeon|Vega|Barcelo|Picasso|Raven'; then
        # First try to extract a clean Radeon name directly
        clean=$(printf '%s\n' "$line" \
            | grep -oE 'Radeon[[:space:]]+([A-Za-z0-9]+[[:space:]]*){1,5}' \
            | head -n1 \
            | sed -E 's/[[:space:]]+$//')

        # Fallback: map some common AMD mobile codenames via CPU model
        if [ -z "$clean" ] || printf '%s\n' "$clean" | grep -qE '^Radeon$|^Radeon Graphics$'; then
            case "$cpu_model" in
        ...
        # If still empty, try bracket content
        if [ -z "$clean" ]; then
            clean=$(printf '%s\n' "$line" \
                | sed -nE 's/.*\[(Radeon[^][]*)\].*/\1/p' \
                | head -n1 \
        ...
        # Normalize vague names
        case "$clean" in
            "Radeon Vega Series / Radeon Vega Mobile Series") clean="Radeon Vega" ;;
            "Radeon Vega Series") clean="Radeon Vega" ;;
            "Radeon Graphics")
                case "$cpu_model" in
                    *"Ryzen 5 7430U"*) clean="Radeon Vega 7" ;;
                    *"Ryzen 7 7730U"*) clean="Radeon Vega 8" ;;
        ...
        # Detect AMD integrated vs dedicated
        if printf '%s\n' "$line" | grep -Eqi 'Barcelo|Picasso|Raven|Renoir|Cezanne|Rembrandt|Phoenix|Vega|Radeon Graphics'; then
            gpu_type="[Integrated]"
        elif printf '%s\n' "$line" | grep -Eqi 'RX|Radeon RX'; then
            gpu_type="[Dedicated]"
        else
            gpu_type="[Integrated]"
        fi

        # Last fallback: shorten raw AMD string
        if [ -z "$clean" ]; then
            clean=$(printf '%s\n' "$line" \
                | sed -E 's/.*AMD\/ATI[[:space:]]*//' \
                | sed -E 's/.*Advanced Micro Devices, Inc\.[[:space:]]*//' \
                | cut -c1-32)
        fi

        [ -n "$clean" ] && echo "$clean $gpu_type" || echo "AMD GPU $gpu_type"
        return
    fi
    ...
    if printf '%s\n' "$line" | grep -qi 'Intel'; then
        ...
        gpu_type="[Integrated]"
        [ -n "$clean" ] && echo "$clean $gpu_type" || echo "Intel Graphics $gpu_type"
        return
    fi
