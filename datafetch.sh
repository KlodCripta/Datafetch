...
# Logo ASCII
logo="\
${GREEN}|${RESET}${BOLD}${GREEN}           ««««««DATAFETCH: Advanced Data Retrieval»»»»»»   ${RESET}${GREEN}|${RESET}
"
...
get_cpu_freq() {
    # Verifica se esiste il file con la frequenza minima della CPU
    if [ -f /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_min_freq ]; then
        base_freq=$(cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_min_freq)
        if [ -z "$base_freq" ]; then
            base_freq="Informazione non disponibile"
        else
            # Converti e arrotonda il valore della frequenza
            base_freq=$(echo "scale=0; $base_freq / 1000" | bc)
            base_freq="${base_freq} MHz"
        fi
    elif [ -f /proc/cpuinfo ]; then
        base_freq=$(grep "cpu MHz" /proc/cpuinfo | awk '{print $4}' | head -n 1)
        if [ -z "$base_freq" ]; then
            base_freq="Informazione non disponibile"
        else
            # Arrotonda il valore
            base_freq=$(echo "scale=0; $base_freq / 1" | bc)
            base_freq="${base_freq} MHz"
        fi
        ...
        base_freq=$(sudo dmidecode -t processor | grep "Current Speed" | awk '{print $3}')
        if [ -z "$base_freq" ]; then
            base_freq="Informazione non disponibile"
        else
            base_freq="${base_freq} MHz"
    ...
    echo -e "${BOLD}${BLUE}Minimal Frequency:${RESET} $base_freq"
}

# Funzione per ottenere la frequenza massima in boost della CPU
get_max_boost_freq() {
    max_boost_freq=$(sudo dmidecode -t processor | grep "Max Speed" | awk '{print $3 " MHz"}')
    echo -e "${BOLD}${BLUE}Max Boost Frequency:${RESET} $max_boost_freq"
}
...
gather_system_info() {
    ...
    echo -e "${GREEN}|____________________________________________________________|${RESET}"
    echo -e "${BLUE}                        Developed by Klod Cripta                        ${RESET}"
    echo -e "${GREEN}|____________________________________________________________|${RESET}"
}
